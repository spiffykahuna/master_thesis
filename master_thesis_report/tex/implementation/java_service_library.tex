...
This section will define a client side of this application.
...
\autoref{fig:rpc_call} showed the main principle of RPC.
Usually RPC client application consists of two modules: client application code
and client RPC stub, that communicates with remote server.
Client calls usual methods on a stub and stub handles all
the communication magic, it sends requests, receives responces, serialize and
deserialize data objects.
...
planform, which has a \gls{JVM} virtual machine inside, and the Java programming
language is used.
...
project and start to develop a new controll application. 
...
This code can be done in two different ways: static and dynamic binding.
This library described here is a static binding library, because it has already 
defined interface class and client methods are known at compile time.

Dynamic libraries can determine available server methods at the runtime. Client
library connects to a server and receives a list of operations. For example JSON
service contract can be obtained and parsed at the runtime. Dynamic proxy
...
I prefered to make more robust solution and created an Java interface class with
all methods that server has. The code is provided below.
...
Some object oriented design techiques are used here to help a programmer to
write more abstract code. \texttt{CoffeeMachineService}  extends another
interface \texttt{Service}, which has method that are common to every RPC
service.
\texttt{CoffeeMachineService} contans only coffee machine related methods and
...
of \texttt{CoffeeMachineService}, connect it to \texttt{Reader} and
\texttt{Writer} stream objects and call RPC methods.
...
not binary data. Binary data only is transported between client and server
hardware and low level j software. \texttt{Reader} and \texttt{Writer} are parents of
...
back using a specified charset\footnote{the mapping between characters and sequences of bytes}.
JVM internally stores characters as 16-bit Unicode variables, but in other
systems character can be:  an US-ASCII seven-bit value, UTF-8 multibyte
...
Message reader class in the library has the implementation of similar finite
state machine parsing algorithm, that was used in the embedded server implementation.

The Message writer class composes a valid netstring message and  uses provided
by the client \texttt{Writer} object to write that message to the destination.
Message writer receives JSON  objects converts them to String data, wraps them
in a netstring message and sends this character data to the desctination.
...
responces 
\autoref{tbl:jsonrpc2-base_methods} showes the entire lifecycle of RPC call.
...
\texttt{jsonrpc2-base} is able to serialize Java datatypes to JSON objects
Listining \ref{lst:jsonrpc2-base_serialization} shows how RPC request object is
created from the standart Java data types and \autoref{tbl:json_java_mapping}
...
\end{minted}
\caption{RPC request creating from Java standart datatypes \cite{jsonrpc2-base}}
...
\begin{table}[h]
	...
	\begin{tabularx}{0.5\textwidth}{|X|X|}
		...
		\hline	  
	\end{tabularx} 
	\caption{{JSON}\(\leftrightarrow\){Java} datatype mapping in
...
When client of our library calls any method on \textt{CoffeeMachineService}
...
After some timeot it returnes the result or a \texttt{null} value if there was
...
be captured by the \texttt{RPCServiceListener}. It also contans a interface of a
...
moving requests, notification, respoces and errors according to their types. 
For example, if \texttt{handleMessage(RPCRequest request)} message handler
method is called from somewhere in the code, message handler publishes recieved
