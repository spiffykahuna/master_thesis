...
\subsubsection{Device connection scheme}
...
provided the specification for internal mechanism of communication to me. There
are several microcontrollers inside that are controlling different machine
parts.
They provided  only the external interface to me, therefore my implementation is
more similar to Proxy approach, where contracts are stored inside Proxy machine
and Proxy is a separate physical device.

TODO  continue with the architecture



\autoref{fig:general_system_arch} shows general  system architecture.
